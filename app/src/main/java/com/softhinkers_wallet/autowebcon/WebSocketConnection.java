...
package com.softhinkers_wallet.autowebcon;
...
import com.softhinkers_wallet.autowebcon.WebSocket.WebSocketConnectionObserver.WebSocketCloseNotification;
import com.softhinkers_wallet.autowebcon.WebSocketMessage.WebSocketCloseCode;
...
//    private static final String WSS_URI_SCHEME = "wss";
    private static final String WS_URI_SCHEME_HTTP = "http";
//    private static final String WSS_URI_SCHEME = "https";
    ...
    public void connect(URI webSocketURI, String[] subprotocols, WebSocketConnectionObserver connectionObserver, WebSocketOptions options) throws WebSocketException {
        Log.i("Connect", "WebsocketConnection.connect() called");
        if (isConnected()) {
            Log.i("Connect", "isConnected() ... already connected");
        ...
        if (webSocketURI == null) {
            ...
            if (!mWebSocketURI.getScheme().equals(WS_URI_SCHEME_HTTP) && !mWebSocketURI.getScheme().equals(WS_URI_SCHEME_HTTP)) {
            ...
            Log.i("Connect", "calling private WebsocketConnection.connect()");
            connect();
        }
    }
    ...
    private void connect() {
        Log.i("Connect", "private WebsocketConnection.connect() called");
        mSocketThread = new SocketThread(mWebSocketURI, mWebSocketOptions);

        mSocketThread.start();
        synchronized (mSocketThread) {
            try {
                Log.i("Connect", "inside synchronized (mSocketThread), waiting on it");
                mSocketThread.wait();
                Log.i("Connect", "inside synchronized (mSocketThread), wait ELAPSED");
            } catch (InterruptedException e) {
            }
        }
        mSocketThread.getHandler().post(new Runnable() {
            ...
            @Override
            public void run() {
                Log.i("Connect", "starting mSocketThread to call startConnection()");
        ...
        synchronized (mSocketThread) {
            try {
                Log.i("Connect", "AFTER startConnection(), inside synchronized (mSocketThread), waiting on it");
                mSocketThread.wait();
                Log.i("Connect", "AFTER startConnection(), inside synchronized (mSocketThread), wait ELAPSED");
            } catch (InterruptedException e) {
            }
        }
    ...
    public static class SocketThread extends Thread {
        ...
        public void startConnection() {
            try {
                ...
                if (port == -1) {
                    if (mWebSocketURI.getScheme().equals(WS_URI_SCHEME_HTTP)) {
                        port = 8080;
                    } else {
                        port = 80;
                    }
                ...
                if (mWebSocketURI.getScheme().equalsIgnoreCase(WS_URI_SCHEME_HTTP)) {
